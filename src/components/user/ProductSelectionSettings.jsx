...
import { ShoppingCart, Grid3X3, List, Eye, CornerUpLeft, Maximize } from 'lucide-react';

export const ProductSelectionSettings = () => {
    const {
        actions: { setProp },
        props
    } = useNode(node => ({
        ...
        <div className="bg-gray-850 text-white p-4 space-y-4 text-sm border-l border-gray-700/50">
            ...
            <div>
                <SectionHeader
                    ...
                    rightIcon={<IconButton icon={ShoppingCart} size="sm" />}
                />
                ...
                <div className="mb-4">
                    ...
                    <div className="flex gap-1">
                        <IconButton
                            ...
                            onClick={() => setProp(p => p.layout = 'grid')}
                        />
                        <IconButton
                            icon={List}
                            active={props.layout === 'list'}
                            onClick={() => setProp(p => p.layout = 'list')}
                        />
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <ToggleSwitch
                        checked={props.showPricing}
                        onChange={val => setProp(p => p.showPricing = val)}
                        label="Show Pricing"
                    />
                    <ToggleSwitch
                        checked={props.showFeatures}
                        onChange={val => setProp(p => p.showFeatures = val)}
                        label="Show Features"
                    />
                </div>
            </div>
            ...
            <div>
                ...
                <div className="mb-4">
                    ...
                    <select
                        ...
                        onChange={e => setProp(p => p.selectedProductId = e.target.value || undefined)}
                        className="w-full px-3 py-2 bg-gray-700/80 border border-gray-600/50 rounded-md text-white text-xs"
                    >
                        <option value="">None Selected</option>
                        <option value="basic">Basic Course Materials</option>
                        <option value="premium">Premium Resources</option>
                        <option value="expert">Expert Package</option>
                    </select>
                </div>
            </div>
